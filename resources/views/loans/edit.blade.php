...
<div style="background: white; padding: 2rem; border-radius: 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
    <div>
        <h2 style="font-size: 2rem; font-weight: 700; color: #1e3a8a; margin: 0;">Edit Loan</h2>
        <p style="color: #6b7280; margin: 0.5rem 0 0 0;">Update loan information - {{ $loan->loan_number }}</p>
    </div>
    <a href="{{ route('loans.show', $loan) }}" class="btn btn-secondary">← Back to Details</a>
</div>
...
<div class="card">
    <form method="POST" action="{{ route('loans.update', $loan) }}">
        ...
        <div class="form-group">
            ...
            <input type="text" name="loan_number" id="loan_number" value="{{ old('loan_number', $loan->loan_number) }}" required>
            @error('loan_number')
        ...
        <div class="form-group">
            ...
            <input type="text" name="customer_name" id="customer_name" value="{{ old('customer_name', $loan->customer_name) }}" required>
            @error('customer_name')
                <div class="error">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input type="number" name="loan_amount" id="loan_amount" step="0.01" value="{{ old('loan_amount', $loan->loan_amount) }}" required>
            @error('loan_amount')
                <div class="error">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input type="number" name="interest_rate" id="interest_rate" step="0.01" value="{{ old('interest_rate', $loan->interest_rate) }}" required>
            @error('interest_rate')
                <div class="error">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input type="number" name="loan_term" id="loan_term" value="{{ old('loan_term', $loan->loan_term) }}" required>
            @error('loan_term')
                <div class="error">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" name="issued_date" id="issued_date" value="{{ old('issued_date', $loan->issued_date->format('d/m/Y')) }}" placeholder="DD/MM/YYYY" required pattern="\d{2}/\d{2}/\d{4}">
            @error('issued_date')
                <div class="error">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" name="due_date" id="due_date" value="{{ old('due_date', $loan->due_date->format('d/m/Y')) }}" placeholder="DD/MM/YYYY" required readonly style="background-color: #f3f4f6; cursor: not-allowed;">
            @error('due_date')
                <div class="error">{{ $message }}</div>
            @enderror
        </div>
